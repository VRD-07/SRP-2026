...
const studentCreateSchema = zod_1.z.object({
    ...
    email: zod_1.z.string().email('Valid email is required').max(150),
    rollNumber: zod_1.z.string().min(2, 'Roll number is required').max(50),
    class: zod_1.z.string().min(1, 'Class is required').max(50),
    batch: zod_1.z.string().min(1, 'Batch is required').max(50),
    admissionYear: zod_1.z.number().int().min(2000).max(2100),
    contactNumber: zod_1.z.string().min(7, 'Contact number is required').max(20),
    password: zod_1.z.string().min(6, 'Default password must be at least 6 characters').max(100).optional(),
});
const assignFeeSchema = zod_1.z.object({
    feeStructureIds: zod_1.z.array(zod_1.z.string().min(1).max(100)).min(1, 'At least one fee structure ID is required'),
});
class StudentController {
    ...
    static async getById(req, res, next) {
        try {
            const { id } = req.params;
            const calculation = await calculationService_1.CalculationService.getStudentFeeCalculation(id);
            if (!calculation) {
    ...
    static async create(req, res, next) {
        try {
            ...
            const existingRoll = await db_1.prisma.student.findUnique({
    ...
    static async update(req, res, next) {
        try {
            ...
            const student = await db_1.prisma.student.findUnique({ where: { id }, include: { user: true } });
            if (!student) {
                res.status(404).json({ success: false, message: 'Student not found' });
                return;
            }
    ...
    static async assignFeeStructures(req, res, next) {
        try {
            const { id } = req.params;
            const { feeStructureIds } = assignFeeSchema.parse(req.body);
            const student = await db_1.prisma.student.findUnique({ where: { id } });
            if (!student) {
                res.status(404).json({ success: false, message: 'Student not found' });
                return;
            }
            await db_1.prisma.feeAssignment.createMany({
            ...
            const updatedSummary = await calculationService_1.CalculationService.getStudentFeeCalculation(id);
            res.status(200).json({
                success: true,
    ...
    static async getMyProfile(req, res, next) {
        try {
            ...
            const summary = await calculationService_1.CalculationService.getStudentFeeCalculation(student.id);
