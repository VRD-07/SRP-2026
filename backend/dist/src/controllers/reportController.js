...
class ReportController {
    ...
    static async getOverviewReport(req, res, next) {
        try {
            ...
            const kpis = await calculationService_1.CalculationService.getSystemKPIs({
                academicYear: academicYear ? String(academicYear) : undefined,
                class: className ? String(className) : undefined,
                ...dateFilter,
            });
            // 2. Breakdown by Fee Head
            const feeHeads = ['Tuition', 'Hostel', 'Transport', 'Exam', 'LateFee', 'Other'];
            const headSummary = await Promise.all(feeHeads.map(async (head) => {
                const assignments = await db_1.prisma.feeAssignment.findMany({
                    where: {
                        feeStructure: {
                            feeHead: head,
            ...
            const classesList = await db_1.prisma.student.findMany({
                distinct: ['class'],
                select: { class: true },
            });
            const classSummary = await Promise.all(classesList.map(async (c) => {
                const assignments = await db_1.prisma.feeAssignment.findMany({
                    where: {
                        feeStructure: {
                            class: c.class,
     ...
     */
    static async getPendingDues(req, res, next) {
        try {
            const { class: className, search } = req.query;
            const dues = await calculationService_1.CalculationService.getStudentsWithOutstandingDues({
                class: className ? String(className) : undefined,
