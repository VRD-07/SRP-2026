...
export class TransactionService {
  private static sequenceInitialized = false;
   ...
   */
  public static async generateReceiptNumber(tx: any, isReversal = false): Promise<string> {
    const now = new Date();
  ...
  static async recordPayment(input: RecordPaymentInput) {
    const cleanAmount = roundCurrency(Number(input.amount));
    ...
    await this.ensureSequencesExist(prisma);
      ...
      async (tx) => {
        ...
        try {
          await tx.$queryRawUnsafe(
            `SELECT id FROM "FeeAssignment" WHERE id = $1 FOR UPDATE`,
        ...
        if (duplicateTx) {
          throw new Error(
            `Duplicate payment detected: A payment of ₹${cleanAmount.toFixed(2)} with reference '${trimmedReference}' was already recorded under receipt ${duplicateTx.receiptNumber}.`
          );
        }
        ...
        for (const t of assignment.transactions) {
          if (t.status === 'SUCCESS' && !t.reversedBy) {
            paidAmount = roundCurrency(paidAmount + Number(t.amount));
          }
        }
        ...
        if (cleanAmount > pendingAmount && !input.allowOverpaymentOverride) {
          throw new Error(
            `Payment amount (₹${cleanAmount.toFixed(2)}) exceeds remaining due (₹${pendingAmount.toFixed(2)}). Confirm overpayment override to proceed.`
          );
        }
   ...
   */
  static async reverseTransaction(input: ReverseTransactionInput) {
    ...
    return await prisma.$transaction(
      async (tx) => {
        ...
        if (originalTx.reversedBy) {
          throw new Error(
            `This transaction was already reversed under receipt ${originalTx.reversedBy.receiptNumber}`
          );
        }
